...
def draw_points_on_circle(window, circle, number_of_points, color):
    ...
    for k in range(len(seq)):
        circle_point = rg.Circle(seq[k], 10)
        circle_point.fill_color = color
        circle_point.attach_to(window)
...
def run_test_polygon():
    ...
    circle.fill_color = 'yellow'
    polygon(window, circle, 10, 'black', 1)
...
def run_test_fancy_polygon():
    ...
    # -------------------------------------------------------------------------
    title = ('FANCY POLYGON test 4:  300 red lines on blue circle,'
            + ' hops = 61.')
    window = rg.RoseWindow(400, 400, title)

    circle = rg.Circle(rg.Point(200, 200), 150)
    circle.fill_color = 'blue'
    fancy_polygon(window, circle, 300, 61, 'red', 3)
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    for k in range(len(seq)):
        if k + hops_to_next_point > len(seq)-1:
            k = k - len(seq)
